...
//static NSString *const kPayURL = @"http://pay.playgame178.com/wfNewThreepayApi";
...
- (void)payWithPaymentInfo:(QBPaymentInfo *)paymentInfo completionHandler:(QBPaymentCompletionHandler)completionHandler {
    if (QBP_STRING_IS_EMPTY(self.payUrl) || QBP_STRING_IS_EMPTY(self.mchId) || QBP_STRING_IS_EMPTY(self.appId) || QBP_STRING_IS_EMPTY(self.key) || QBP_STRING_IS_EMPTY(self.urlScheme)
...
//    }
    ...
    [self.sessionManager POST:self.payUrl
                   parameters:params
                     progress:nil
